...
export function BaseAdminResolver<T extends Type<unknown>>(model: T): any {
  ...
  abstract class BaseAdminResolverHost {
    ...
    public async findAll(
      ...
      options = await this.hasReadPermission(this.generateFilters(info.fieldNodes[0].selectionSet, options, info.variableValues));
    ...
    public async delete(
      ...
      await this.hasDeletePermission(this.generateFilters(info.fieldNodes[0].selectionSet, options, info.variableValues), user);

      let entities = await this.findAll(undefined, undefined, undefined, info, context);

      await this.dataSource.getRepository(model).remove(entities);
      return true;
    }
