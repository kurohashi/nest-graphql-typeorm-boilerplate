...
export class ZohoHelper {
  ...
  async createSalesOrder(lineItems: LineItem[], salesorder_number, outlet_id): Promise<any> {
    ...
    const tokenData = await this.generateAccessToken(this.optionsRefresh);
    console.log(salesorder_number.salesorder_number, salesorder_number[0].salesorder_number);
    const newSalesOrderNumber = await this.incrementSalesOrderNumber(salesorder_number[0].salesorder_number);
