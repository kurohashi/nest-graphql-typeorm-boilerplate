...
import { client as redisClient } from "../customer/utils/redis.config"
...
export class SalesOrderService {
    ...
    constructor(
        ...
        console.log('SalesOrderService Initiated!');
    }
    @Cron('45 * * * * *')
    async createSalesOrdersInZoho() {
        try {
            ...
            for (const order of orders) {
                ...
                const lineItemsForOrder = items.map(item => this.constructLineItem(item));
    ...
    private constructLineItem(item: Item) {
        return {
            item_id: parseInt(item.item_id_zoho),
    ...
    async createZohoCOntact() {
        try {
            ...
            for (const outlet of outletsToProcess) {
                ...
                const shippingAddress = await this.addressRepository.findOne({ where: { id: outlet.address_id } });
